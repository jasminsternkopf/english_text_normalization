...
def init_file_stem_loggers(file_stems: Set[str]) -> Dict[str, Queue]:
...
def init_file_stem_logger_lists(file_stems: Set[str]) -> Dict[str, List[Tuple[int, str]]]:
  logging_queues = dict.fromkeys(file_stems)
  for k in file_stems:
    logging_queues[k] = []
  return logging_queues
...
def get_file_stem_loggers(file_stems: Set[str]) -> Generator[Logger, None, None]:
  for k in file_stems:
    logger = getLogger(k)
    yield logger
